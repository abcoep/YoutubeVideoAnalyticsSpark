package com.amitbongir.bigdata
...
/**
 * Author: Amit Shekhar Bongir
 *
 */
object YoutubeVideoAnalyticsSpark {
...
//			.master("local[*]")
		...
		val root_dir = "abfs://topics@youtubevideoanalytics.dfs.core.windows.net/"
		val resources_dir = root_dir
		...
		val categoryTitleDf = sqlContext.read.format("avro").option("header", "true").option("inferSchema", "true")
			.load(resources_dir + "category_title*/*/category_title*.avro")
		...
		var usVideosDf = sqlContext.read.format("avro").option("header", "true").option("inferSchema", "true")
			.load(resources_dir + "USvideos*/*/USvideos*.avro").join(categoryTitleDf, Seq("category_id"), "left")
